...
//|    Generated at Tue Jan 23 05:22:00 GMT 2018
//|
//|    Tested on AUDUSD, M1, 01.01.2004 - 01.01.2016
...
extern string CustomComment = "Strategy 6.18";
...
int start() {
   ...
   // ENTRY RULES
      ...
      // LONG: (SMA(54) > (WMA(11) * EMA(19)))
      if(TradeLong) {
         bool LongEntryCondition = (iMA(NULL, 0, 54, 0, MODE_SMA, PRICE_CLOSE, 1) > (iMA(NULL, 0, 11, 0, MODE_LWMA, PRICE_CLOSE, 1) * iMA(NULL, 0, 19, 0, MODE_EMA, PRICE_CLOSE, 1)));
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: (SMA(54) < (WMA(11) * EMA(19)))
      if(TradeShort) {
         bool ShortEntryCondition = (iMA(NULL, 0, 54, 0, MODE_SMA, PRICE_CLOSE, 1) < (iMA(NULL, 0, 11, 0, MODE_LWMA, PRICE_CLOSE, 1) * iMA(NULL, 0, 19, 0, MODE_EMA, PRICE_CLOSE, 1)));
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 6.18", 9, "Tahoma", LabelColor);
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(40 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(40 * gPointCoef));
   }
